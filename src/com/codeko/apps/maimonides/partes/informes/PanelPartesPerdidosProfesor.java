 ...
 * @author  Codeko
 */
public class PanelPartesPerdidosProfesor extends javax.swing.JPanel implements IPanel {
    ...
    PanelPartesPerdidosProfesor auto = this;
    boolean cargado = false;

    /** Creates new form PanelPartesPendientes */
    public PanelPartesPerdidosProfesor() {
        initComponents();
        MaimonidesUtil.addMenuTabla(tabla, "Partes pendientes de digitalizar");
        MaimonidesUtil.addMenuTabla(tablaProfesores, "Relación de partes pendientes de digitalizar agrupados por profesor");
    ...
    private void initComponents() {
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance(com.codeko.apps.maimonides.MaimonidesApp.class).getContext().getActionMap(PanelPartesPerdidosProfesor.class, this);
    ...
    public static int getTotalPartes(int idProfesor) {
        ...
        try {
            String sql = "select count(*) AS total FROM partes AS p "
                    + " JOIN partes_horarios AS pa ON pa.parte_id=p.id "
                    + " JOIN horarios AS h ON h.id=pa.horario_id "
                    + " JOIN profesores AS prof ON prof.id=h.profesor_id "
                    + " WHERE p.ano=? AND p.digitalizado=0 AND DATE(p.fecha)< DATE(NOW()) AND prof.id=? ";
            ...
            Logger.getLogger(PanelPartesPerdidosProfesor.class.getName()).log(Level.SEVERE, null, ex);
        }
            ...
            if (p != null) {
                ...
                try {
                    ...
                    String sql = "select distinct p.* FROM partes_horarios AS pa "
                            ...
                            + " WHERE p.ano=? AND h.ano=? AND pf.ano=? AND p.digitalizado=0 AND DATE(p.fecha)< DATE(NOW()) AND pf.id=? GROUP BY p.id "
                    ...
                    while (res.next() && !isCancelled()) {
                        pos++;
                        setMessage(String.format("Cargando partes de asistencia %d de %d...", pos, total));
    ...
    private class ActualizarProfesoresTask extends org.jdesktop.application.Task<Object, Void> {
        ...
        protected Object doInBackground() {
            ...
            try {
                ...
                String sql = "SELECT prof.*,count(distinct p.id) AS total FROM partes AS p "
                        + " JOIN partes_horarios AS pa ON pa.parte_id=p.id "
                        + " JOIN horarios AS h ON h.id=pa.horario_id "
                        + " JOIN profesores AS prof ON prof.id=h.profesor_id "
                        + " WHERE p.ano=? AND p.digitalizado=0 AND DATE(p.fecha)< DATE(NOW()) "
                        + " GROUP BY prof.id ORDER BY total DESC";
                st = (PreparedStatement) MaimonidesApp.getApplication().getConector().getConexion().prepareStatement(sql);
                st.setInt(1, MaimonidesApp.getApplication().getAnoEscolar().getId());
                res = st.executeQuery();
                int pos = 0;
                while (res.next() && !isCancelled()) {
                    ...
                    ProfesorPartesNoFirmado pn = new ProfesorPartesNoFirmado(p, res.getInt("total"));
